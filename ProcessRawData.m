...
%
% For processing single session, call ProcessRawData({'Ivan', 22})
%
    ...
    for f = 1 : length(proc_folders)
        ...
        c_id = regexp({files.name}, ['^' proc_folders{f} '\.R?G?M?\.?(\d+)'], 'tokens');
        ...
        for r = ix
            ...
            fprintf('> %s\n', proc_dir);
            try
                Preprocess_EMG_Data({proc_folders{f}, id(r)});
                Preprocess_Plexon_EMG({proc_folders{f}, id(r)});
                Preprocess_Plexon_Neural({proc_folders{f}, id(r)});

                iD_out_file = [Intermediate_Folder strrep(files(r).name, 'RGM.', '') '.mat'];
                ...
                iF_out_file = [Formatted_Folder strrep(files(r).name, 'RGM.', '')  '.mat'];
